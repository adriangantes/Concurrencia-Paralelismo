...
comp_loop(Reader, Writer, My_Pid) ->  %% Compression loop => get a chunk, compress it, send to writer
    Reader ! {get_chunk, self()},  %% request a chunk from the file reader
    receive
        {chunk, Num, Offset, Data} ->   %% got one, compress and send to writer
            Comp_Data = compress:compress(Data),
            Writer ! {add_chunk, Num, Offset, Comp_Data},
            comp_loop(Reader, Writer, My_Pid);
        eof ->  %% end of file, stop reader and writer
            My_Pid ! {finish, self()};
        {error, Reason} ->
            io:format("Error reading input file: ~w~n",[Reason]),
...
comp_proc(File, Chunk_Size, Procs) ->
    case file_service:start_file_reader(File, Chunk_Size) of
        {ok, Reader} ->
            case archive:start_archive_writer(File++".ch") of
                {ok, Writer} ->
                    Pids = spawn_compressors(Procs, Reader, Writer),
                    wait_for_completion(Pids, Reader, Writer);
                {error, Reason} ->
                    io:format("Could not open output file: ~w~n", [Reason])
            end;
        {error, Reason} ->
...
spawn_compressors(0, _, _) -> [];
spawn_compressors(NumProcs, Reader, Writer) ->
    My_Pid = self(),
    Pid = spawn(fun() -> comp_loop(Reader, Writer, My_Pid) end),
    [Pid | spawn_compressors(NumProcs - 1, Reader, Writer)].

wait_for_completion([], Reader, Writer) ->
    Reader ! stop,
    Writer ! stop;

wait_for_completion(Pids, Reader, Writer) ->
    receive
        {finish, Pid} -> wait_for_completion(Pids--[Pid], Reader, Writer)
    end.
...
decomp_proc(Archive, Output_File, Procs) ->
    case archive:start_archive_reader(Archive) of
        {ok, Reader} ->
            case file_service:start_file_writer(Output_File) of
                {ok, Writer} ->
                    Pids = spawn_decomp(Procs, Reader, Writer),
                    wait_for_completion(Pids, Reader, Writer);
                {error, Reason} ->
                    io:format("Could not open output file: ~w~n", [Reason])
            end;
...
decomp_loop(Reader, Writer, My_Pid) ->
    Reader ! {get_chunk, self()},  %% request a chunk from the reader
    receive
        {chunk, _Num, Offset, Comp_Data} ->  %% got one
            Data = compress:decompress(Comp_Data),
...
spawn_decomp(0, _, _) -> [];
spawn_decomp(NumProcs, Reader, Writer) ->
    My_Pid = self(),
    Pid = spawn(fun() -> decomp_loop(Reader, Writer, My_Pid) end),
    [Pid | spawn_decomp(NumProcs - 1, Reader, Writer)].
